...
import { getWarehousesDataAction, setSelectedWarehouse } from './redux/actions/actionCreator';
    ...
    return (
      <div className={styles.wrap}>
        <div className={styles.componentWrap}>
          ...
          onClick={() => warehousesData.length > 0 ? setWarehousesOpen(!warehouseOpen) : void 0}>
          ...
          style={{display: warehouseOpen ? 'block' : 'none'}}>
            <div className={styles.placeholder}>
              ...
              <input 
                ...
                onChange={(e) => setWarehousesInputValue(e.target.value.toLowerCase())}
                placeholder={(warehousesData?.length > 0 ? 'Введите название отделения' : 'Нет доступных отделений')}
                className={styles.input}/>
            </div>
              <div className={styles.list} >
              {warehousesData.map((warehouse) => (
                      <option 
                      key={warehouse.Ref} value={warehouse.Description}
                      className={`
                      ${styles.li} 
                      ${warehouse.Description.toLowerCase() === selectedWarehouse.toLowerCase()? styles.selectedElement : ''}`}
                      style={{display: new RegExp(warehousesInputValue, 'i')
                      .test(warehouse.Description) ? 'block' : 'none'
  
                    }}
                      onClick={()=> {
                        if (warehouse.Description.toLowerCase() !== selectedWarehouse.toLowerCase()) {
...
export default connect(mapStateToProps, mapDispatchToProps)(NPWarehouseSelection);